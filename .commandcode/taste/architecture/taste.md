...
- For anti-spam, use honeypot + Turnstile (backend-validated) + Cloudflare Rate Limiting Rule on /api/lead endpoint — not in-memory rate limiting in middleware (Pages Functions run in many isolates, Map() is insufficient). Confidence: 0.80
